...
def download_and_parse_mnist_file(fname, target_dir=None, force=False):
    ...
    fopen = gzip.open if os.path.splitext(target_dir + fname)[1] == '.gz' else open
    with fopen(target_dir + fname, 'rb') as fd:
        return parse_idx(fd)


def train_images(data_dir):
...
def train_labels(data_dir):
    ...
    """
    return dense_to_one_hot(download_and_parse_mnist_file('train-labels-idx1-ubyte.gz', data_dir))
...
def test_labels(data_dir):
    ...
    test_labels : numpy.ndarray
        Numpy array with the labels 0 to 9 in the train MNIST database.
    """
    return dense_to_one_hot(download_and_parse_mnist_file('t10k-labels-idx1-ubyte.gz', data_dir))
